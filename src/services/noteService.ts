...
export const getNotesByAuthorId = async () => {
  ...
  const res = await axiosClient.get(`/notes/author?userId=${userId}`);
  return res.data;
};

export const createNote = async (node: CreateNoteRequest) => {
  const res = await axiosClient.post("/notes", node);
  return res.data.id;
};
...
export const updateNote = async (id: string, node: CreateNoteRequest) => {
  const res = await axiosClient.put(`/notes/${id}`, node);
  return res.data;
};
...
export const getNotesByAuthorIdAndTitle = async (title: string) => {
  ...
  const res = await axiosClient.get(
    `/notes/author?userId=${userId}&filter=${title}`
  );
  return res.data;
};
